...
def predict_with_fold_model(model_dir, tokenizer, test_ds_mapped, test_ds_rev_mapped, device, use_bf16):
    """Loads one fold's saved model and returns (standard_logits, reversed_logits)."""
    model = build_model(model_dir, device)
...
def main():
    ...
    for fold_dir in fold_dirs:
        ...
        model_path = os.path.join(args.saved_models_dir, fold_dir)
        std_logits, rev_logits = predict_with_fold_model(
            model_path, tokenizer, test_ds_mapped, test_ds_rev_mapped, device, use_bf16
        )
        all_test_preds.append(std_logits)
        all_test_preds_rev.append(rev_logits)
